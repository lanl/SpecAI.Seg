...
# from ..metrics import distance_cy as dist
...
def _slic_cython(np_floats[:, :, :, ::1] image_zyx,
    ...
    ignore_color : bool
        ...
        color.
    m2 : float
        The squared compactness parameter.
    Returns
    -------
    ...
    cdef np_floats numer, denom_1, denom_2, denom, dist
    cdef np_floats[:] s1, s2
                ...
                for z in range(z_min, z_max):
                    ...
                    for y in range(y_min, y_max):
                        ...
                        for x in range(x_min, x_max):
                            ...
                            dx = sx * (cx - x)
                            dx *= dx
                            dist_center = (dz + dy + dx) * spatial_weight * m2

                            if not ignore_color:
                                # Calculates cosine distance
                                s1 = image_zyx[z, y, x, 0:(n_features - 3)]
                                s2 = segments[k, 3:n_features]
                                numer = 0.0
                                denom_1 = 0.0
                                denom_2 = 0.0
