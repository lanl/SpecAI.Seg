...
def slic(image, n_segments=100, compactness=10., max_num_iter=10, sigma=0,
    ...
    * `start_label` is introduced to handle the issue [4]_. Label indexing
    ...
    segments = np.ascontiguousarray(np.concatenate(
        ...
        axis=-1), dtype=dtype)
    # initializes centroids to not be all 0's for cosine distance
    for r in range(segments.shape[0]):
        z, y, x = segments[r, 0], segments[r, 1], segments[r, 2]
        segments[r, 3:] = image[int(z), int(y), int(x), :]

    # Squares m here for use in cython to find distance from centroid
    step = max(steps)
    m = compactness * compactness
